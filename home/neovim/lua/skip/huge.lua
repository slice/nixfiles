...
function M.bounce(bufnr, reason, opts)
  ...
  if utils.flag_set(M.bypass_key, bufnr) then
    if not silent then
      vim.notify(('huge: not bouncing this buffer (%d), bypass flag set'):format(bufnr), vim.log.levels.INFO)
    end
    return
  end
  ...
  vim.schedule(function() vim.b[bufnr].current_syntax = '' end) -- and i'd do it again
  vim.schedule(function() vim.cmd [[syntax clear]] end)         -- and i'd do it again

  local formatted_reason = reason and ('reason: %s'):format(reason) or 'no resaon'
  if not silent then
    vim.notify(('huge: BOUNCING buffer %d (%s)'):format(bufnr, formatted_reason), vim.log.levels.WARN)
  end
