...
function M.look(selected)
  ...
  vim.system({ "gh", "api", selected.api_url, "-q", ".download_url" }, { text = true }, function(completed)
    if completed.code ~= 0 then
      vim.schedule(function()
        pcall(notif.cancel, notif)
        vim.notify(("couldn't query download url (curl exited with %d)"):format(completed.code), vim.log.levels.ERROR)
      end)
      return
    end

    -- lol javascript amirite
    notif.message = ("downloading %s from %s..."):format(selected.path, selected.repo)
    ...
    process, pid = vim.uv.spawn("curl", {
      ...
      if code ~= 0 then
        vim.schedule_wrap(vim.notify)("curl died with code " .. tostring(code) .. " :(", vim.log.levels.ERROR)
        return
      end
      ...
      vim.schedule(function()
        ...
        local ft, apply_filetype = vim.filetype.match({ buf = buf, filename = selected.name })
        ...
        if not ok then
          vim.schedule_wrap(vim.notify)(
            ("couldn't focus buffer %d from looking-glass: %s"):format(buf, err),
